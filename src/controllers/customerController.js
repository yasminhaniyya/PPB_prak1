import { CustomerModel } from "../models/customerModel.js";
export const CustomerController = {
async getAll(req, res) {
try {
const { name, page, limit } = req.query; // Ambil parameter nama, page, dan limit dari query URL

// Tetapkan nilai default: halaman 1, maksimal 10 data per halaman (jika tidak diisi)
const pageNumber = parseInt(page) || 1;
const limitNumber = parseInt(limit) || 10;

const customers = await CustomerModel.getAll(name, pageNumber, limitNumber); // Kirim ke model
res.json(customers);
} catch (err) {
res.status(500).json({ error: err.message });
}
},
async getById(req, res) {
try {
const customer = await CustomerModel.getById(req.params.id);
res.json(customer);
} catch (err) {
res.status(404).json({ error: err.message });
}
},
async create(req, res) {
try {
const { email, phone } = req.body;

// Validasi Email
if (email && !email.includes("@")) {
return res.status(400).json({ error: "Email tidak valid. Harus mengandung karakter '@'." });
}

// Validasi Phone
if (phone && phone.length < 10) {
return res.status(400).json({ error: "Nomor telepon harus diisi minimal 10 karakter." });
}

const customer = await CustomerModel.create(req.body);
res.status(201).json(customer);
} catch (err) {
res.status(400).json({ error: err.message });
}
},
async update(req, res) {
try {
const { email, phone } = req.body;

// Validasi Email
if (email && !email.includes("@")) {
return res.status(400).json({ error: "Email tidak valid. Harus mengandung karakter '@'." });
}

// Validasi Phone
if (phone && phone.length < 10) {
return res.status(400).json({ error: "Nomor telepon harus diisi minimal 10 karakter." });
}

const customer = await CustomerModel.update(req.params.id, req.body);
res.json(customer);
} catch (err) {
res.status(400).json({ error: err.message });
}
},
async remove(req, res) {
try {
await CustomerModel.remove(req.params.id);
res.json({ message: "Customer deleted successfully" });
} catch (err) {
res.status(400).json({ error: err.message });
}
},
};